...
export function DeliveryMap({ deliveryPoints, scrollZoom = false, showPolyline = false, markerStyle = "pin", mapStyle = "osm", startPoint, includeStartInBounds = true, refitToken = 0, resizeToken = 0, onResetRoute, focusPoint, focusToken = 0 }: DeliveryMapProps) {
  ...
  return (
    <div ref={containerRef} style={{ position: 'relative', width: '100%', height: '100%' }}>
      ...
      {(initialBounds || onResetRoute) && (
        <div style={{ position: 'absolute', top: 10, right: 10, zIndex: 1000, display: 'flex', flexDirection: 'column', gap: 5, opacity: controlsVisible ? 1 : 0, transform: controlsVisible ? 'translateY(0)' : 'translateY(-6px)', transition: 'opacity 0.25s ease, transform 0.25s ease', pointerEvents: controlsVisible ? 'auto' : 'none' }}>
          {initialBounds && (
            <button
              type="button"
              onClick={resetView}
